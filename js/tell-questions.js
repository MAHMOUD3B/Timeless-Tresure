...
let choiseNum = 0;
let questionNumber = JSON.parse(localStorage.getItem("tellQuestionNumber"));
...
function nextQuestion() {
  if (questionNumber < questions.length) {
    questionNumber++;
    localStorage.setItem("tellQuestionNumber", questionNumber);
    location.reload();
  } else {
    questionsBox.classList.add("d-none");
  }
...
function creatReport() {
  if (questionNumber === questions.length) {
    ...
    localStorage.setItem("place1", "تل العمارنة");
  }
}
generateQuestions();
