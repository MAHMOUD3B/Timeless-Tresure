...
let choiseNum = 0;
let questionNumber = JSON.parse(localStorage.getItem("ashmoninQuestionNumber"));
...
function nextQuestion() {
  if (questionNumber < questions.length) {
    questionNumber++;
    localStorage.setItem("ashmoninQuestionNumber", questionNumber);
    location.reload();
  } else {
    questionsBox.classList.add("d-none");
  }
...
function creatReport() {
  if (questionNumber === questions.length) {
    ...
    localStorage.setItem("place3", "الاشمونين");
  }
}
generateQuestions();